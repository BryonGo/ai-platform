...
import { myActorPath } from '~/utils/routes'
...
onMounted(async () => {
  ...
  try {
    character.value = await api.getCharacter(characterId.value)
  } catch (e: unknown) {
...
async function onSubmit(input: CharacterInput) {
  ...
  try {
    ...
    await navigateTo(myActorPath(characterId.value))
  } catch (e: unknown) {
  ...
  <div class="page-body">
    ...
    <template v-else-if="character">
      <AppCharacterForm
        ...
        :server-error="error"
        :cancel-to="myActorPath(characterId)"
        edit-mode
        submit-label="保存修改"
        @submit="onSubmit"
      />
