...
- 导出函数/常量：`useHougongApi()` L746
- 导出类型 57 个 → 字段见 `types.md`（`AuthResult` · `AppearanceItem` · `OutfitItem` · `CharacterItem` · `CharacterInput` · `WorkItem` …）

### `useHougongApi()` — L746 · 返回 101 个成员
- `login(email: string, password: string, turnstileToken?: string): Promise<AuthResult>` · L754 · `/account/auth/login` — 登录只认邮箱（后端口径，2026-09）：用户名是自动生成、可自行修改的展示名， 不再作为登录标识，所以这里发的字段就是 email，而不是过去的 acco…
- `register(input: { email: string, password: string, turnstileToken?: string }): Promise<AuthResult>` · L772 · `/account/auth/register` — 邮箱注册
- `getProfile(): Promise<ProfileInfo>` · L789 · `/account/profile` — 当前账号信息（用户名/邮箱/展示名）
- `updateUsername(username: string): Promise<{ username: string, nickname: string }>` · L797 · `/account/profile/edit` — 改用户名
- `logout()` · L804
- `listCharacters(): Promise<CharacterItem[]>` · L808 · `/hougong/characters`
- `getCharacter(id: number): Promise<CharacterItem>` · L813 · `/hougong/characters/…`
- `createCharacter(input: CharacterInput): Promise<CharacterItem>` · L818 · `/hougong/characters` — 角色创建/更新（字段对齐后端 CharacterInputData
- `updateCharacter(id: number | string, input: CharacterInput): Promise<CharacterItem>` · L822 · `/hougong/characters/…`
- `setCharacterCover(id: number | string, assetId: number | string): Promise<CharacterItem>` · L827 · `/hougong/characters/…` — 指定 / 清除角色封面（assetId=0 恢复自动：取最近作品产物），返回更新后的角色
- `deleteCharacter(id: number | string): Promise<void>` · L836 · `/hougong/characters/…` — 删除角色：后端软删
- `listWorks(): Promise<WorkItem[]>` · L840 · `/hougong/works`
- `getHougongWork(id: string | number): Promise<WorkItem>` · L846 · `/hougong/works/…` — 后宫作品详情/收藏（与 /platform/work 的发布作品区分开）
- `favoriteHougongWork(id: string | number, favorite: boolean): Promise<WorkItem>` · L850 · `/hougong/works/…`
- `deleteHougongWork(id: string | number): Promise<void>` · L854 · `/hougong/works/…`
- `listStories(): Promise<StoryItem[]>` · L858 · `/hougong/stories`
- `getHougongStory(id: string | number): Promise<StoryItem>` · L863 · `/hougong/stories/…`
- `updateStoryClips(id: string | number, clips: { workId: number, order: number, note: string }[]): Promise<StoryItem>` · L868 · `/hougong/stories/…` — 整理片段：全量替换（按数组顺序重排 order
- **集（画布·集）：画布按 ownerType=episode 挂图，集本身由产品管**
  - `listEpisodes(storyId: string | number): Promise<EpisodeItem[]>` · L873 · `/hougong/stories/…`
  - `createEpisode(storyId: string | number, body: { idx?: number, title?: string } = {}): Promise<EpisodeItem>` · L879 · `/hougong/stories/…` — 新建一集：`idx` 不传 = 服务端取"这一故事下一个"，`title` 空 = "第 N 集"
  - `updateEpisode(id: string | number, body: { title?: string, status?: string, budgetCredits?: number }): Promise<EpisodeItem>` · L884 · `/hougong/episodes/…` — 改一集：只改给到的字段（改名不会把状态退回 todo）
  - `deleteEpisode(id: string | number): Promise<void>` · L888 · `/hougong/episodes/…`
  - `listTasks(): Promise<HougongTask[]>` · L892 · `/hougong/tasks`
  - `wallet(): Promise<{ balance: number, holds: number }>` · L897 · `/account/credits`
  - `listTools(): Promise<ToolCatalogItem[]>` · L909 · `/hougong/tools` — 创作工具目录（本站已启用的工具与模板）
  - `getCatalog(includeUnavailable = false): Promise<Catalog>` · L914 · `/platform/catalog…`
  - `optimizePrompt(prompt: string, modelId: string): Promise<string>` · L919 · `/hougong/prompt/optimize`
  - `translatePrompt(prompt: string, target = 'en'): Promise<string>` · L924 · `/hougong/prompt/translate`
  - `uploadAsset(file: File): Promise<{ assetId: string, width: number, height: number, mimeType: string }>` · L932 · `/platform/asset` — 上传资产
  - `createTask(input: { clientKey: string, type: string, prompt: string, negativePrompt?: string, ratio?: string, width?: number, height?: number, count?: number, durationSeconds?: number, seed?: number, sampling?: { steps: number, sampler: string, scheduler: string, cfg: number, denoise?: number }, loras?: { name: string, weight: number }[], modelId?: string, quality?: string, engine?: string, characterId?: string, refAssetIds?: string[], tool?: string, template?: string }): Promise<HougongTask>` · L941
  - `getTask(id: number | string): Promise<HougongTask>` · L967 · `/hougong/tasks/…`
  - `cancelTask(id: number | string): Promise<{ status: string }>` · L971 · `/hougong/tasks/…`
  - `retryTask(id: number | string, clientKey: string): Promise<{ id: number, status: string }>` · L978 · `/hougong/tasks/…` — 重试任务
  - `createWork(input: WorkCreateInput): Promise<WorkItem>` · L982 · `/hougong/works`
  - `setWorkVisibility(id: number | string, visibility: 'private' | 'unlisted' | 'public', contentRating?: 'sfw' | 'r15' | 'r18'): Promise<WorkItem>` · L987 · `/hougong/works/…` — 修改作品可见性（可选一并改分级）
- **生成会话（workspace）**
  - `listSessions(page = 1, pageSize = 20): Promise<SessionItem[]>` · L999 · `/platform/session?page=…`
  - `createSession(title?: string): Promise<SessionItem>` · L1003 · `/platform/session`
  - `getSession(id: string): Promise<SessionItem>` · L1007 · `/platform/session/…`
  - `renameSession(id: string, title: string): Promise<SessionItem>` · L1010 · `/platform/session/…`
  - `archiveSession(id: string): Promise<SessionItem>` · L1013 · `/platform/session/…`
  - `listSessionTasks(id: string, page = 1, pageSize = 20): Promise<HougongTask[]>` · L1016 · `/platform/session/…`
- **快捷词（snippet）**
  - `snippetCategories(): Promise<SnippetCategory[]>` · L1022 · `/platform/snippet/categories`
  - `snippetList(input: { category: string, subcategory?: string, query?: string, cursor?: string, limit?: number }): Promise<{ items: SnippetItem[], nextCursor?: string }>` · L1026 · `/platform/snippet…`
- **资产库（asset）**
  - `listAssets(q: { hidden?: boolean, kind?: string, origin?: string, scope?: 'all' | 'permanent' | 'temp', keyword?: string, sort?: 'new' | 'old' | 'large', duplicates?: boolean, page?: number, pageSize?: number } = {}): Promise<{ items: AssetItem[], total: number }>` · L1044 — 资产库列表
  - `batchAssets(ids: string[], action: 'delete' | 'hide' | 'unhide'): Promise<{ ok: boolean, affected: number, failed: { id: string, reason: string }[] }>` · L1082 · `/platform/asset/batch` — 批量操作资产（删除/隐藏/恢复）
  - `dedupeAssets(dryRun = false): Promise<{ groups: number, deleted: number, kept: number, mergedTasks?: number }>` · L1095 — 清理重复素材（同内容多行）
  - `removeAsset(id: string): Promise<void>` · L1104 · `/platform/asset/…`
  - `setAssetHidden(id: string, hidden: boolean): Promise<void>` · L1107 · `/platform/asset/…`
  - `saveAsset(id: string): Promise<AssetItem>` · L1116 · `/platform/asset/…` — saveAsset 把临时生成产物「保存到我的资产」：后端把 scope 置为 permanent（幂等）
  - `assetSelect(page = 1, pageSize = 20): Promise<AssetChoice[]>` · L1120 · `/platform/asset/select?page=…`
  - `assetSelectByIds(ids: string[]): Promise<AssetChoice[]>` · L1124 · `/platform/asset/selectByIds`
  - `assetDownloadUrl(id: string): Promise<{ url: string, expiresAt: string }>` · L1132 · `/platform/asset/…` — assetDownloadUrl 取资产的原图下载地址（签发限时 URL）
- **导出（工程包）**
  - `createExport(items: { assetId: string, relPath?: string }[], sourceKind?: string, sourceId?: string): Promise<{ exportId: string }>` · L1139 · `/platform/export` — 把一组资产按 relPath 打成 ZIP，**异步**执行：创建后轮询 getExport 到 succeeded， 再取 downloadUrl
  - `getExport(id: string): Promise<{ found: boolean, task?: ExportTask }>` · L1149 · `/platform/export/…`
  - `listExports(page = 1, pageSize = 20): Promise<ExportTask[]>` · L1152 · `/platform/export?page=…`
- **发布（work/post/comment/tag/report）**
  - `listWorksFeed(page = 1, pageSize = 20, scope: 'explore' | 'owned' | 'favorites' = 'explore'): Promise<{ items: PublicationWork[], total: number }>` · L1165 · `/platform/work?scope=…` — 作品流（灵感广场）
  - `getWork(id: string): Promise<PublicationWork>` · L1175 · `/platform/work/…`
  - `getWorkEditor(id?: string | number): Promise<WorkEditor | null>` · L1185 · `/platform/work/edit…` — 取作品编辑器（缺省 id 取当前草稿，可能为 null）
  - `saveWorkDraft(input: WorkSaveInput): Promise<WorkEditor | null>` · L1196 · `/platform/work/edit` — 保存作品草稿（不发布）
  - `publishWork(id?: string | number): Promise<PublicationWork>` · L1212 · `/platform/work/publish` — 发布作品（缺省 id 发布当前草稿），返回发布后的作品详情（含 coverUrl）
  - `unpublishWork(id: string | number): Promise<void>` · L1218 · `/platform/work/…` — 取消发布（published → unpublished）
  - `setWorkHidden(id: string | number, hidden: boolean): Promise<void>` · L1223 · `/platform/work/…` — 隐藏/恢复作品（作者本人或管理员）
  - `deleteWork(id: string | number): Promise<void>` · L1228 · `/platform/work/…` — 删除作品
  - `listMyPublishedWorks(page = 1, pageSize = 20): Promise<{ items: PublicationWork[], total: number }>` · L1233 — 我发布的作品（scope=owned，含草稿/未发布/隐藏状态）
  - `listPosts(page = 1, pageSize = 20): Promise<PublicationPost[]>` · L1237 · `/platform/post?page=…`
  - `listComments(targetKind: string, targetId: string): Promise<PublicationComment[]>` · L1241 · `/platform/comment?targetKind=…`
  - `createComment(input: { targetKind: string, targetId: string, content: string, parentId?: string }): Promise<PublicationComment>` · L1245 · `/platform/comment`
  - `react(input: { targetKind: string, targetId: string, kind: string }): Promise<void>` · L1248 · `/platform/interaction/reaction`
  - `searchTags(query: string): Promise<PlatformTag[]>` · L1251 · `/platform/tag/search?query=…`
  - `createTag(name: string): Promise<PlatformTag>` · L1255 · `/platform/tag` — 创建标签（POST /platform/tag）
  - `report(input: { targetKind: string, targetId: string, reason: string, detail?: string }): Promise<void>` · L1258 · `/platform/report`
- **经济（wallet/invite/membership/checkout/transaction/creator）**
  - `walletBalance(): Promise<EconomyWallet>` · L1263 · `/platform/economy/wallet`
  - `claimDaily(): Promise<EconomyWallet>` · L1266 · `/platform/economy/wallet/claim`
  - `walletLedger(asset?: string, kind?: string, page = 1, pageSize = 20): Promise<WalletLedgerItem[]>` · L1269 · `/platform/economy/wallet/ledger…`
  - `invite(): Promise<Invite>` · L1276 · `/platform/economy/invite`
  - `membership(): Promise<Membership>` · L1279 · `/platform/economy/membership`
  - `creator(): Promise<Creator>` · L1282 · `/platform/economy/creator`
  - `submitCreator(input: { direction: string, statement: string, workIds: string[], agreed: boolean }): Promise<Creator>` · L1285 · `/platform/economy/creator/submit`
  - `modelCreator(): Promise<ModelCreator>` · L1288 · `/platform/economy/model-creator`
  - `submitModelCreator(input: { platform: string, profileUrl: string, resourceUrls: string[], agreed: boolean }): Promise<ModelCreator>` · L1291 · `/platform/economy/model-creator/submit`
  - `listTransactions(page = 1, pageSize = 20): Promise<Transaction[]>` · L1294 · `/platform/economy/transaction?page=…`
  - `checkoutCreate(purchase: { kind: string, yuan?: number, tier?: string, choice?: string }, clientKey = 'web-' + Date.now()): Promise<Purchase>` · L1298 · `/platform/economy/checkout`
  - `checkoutGet(id: string): Promise<Purchase>` · L1301 · `/platform/economy/checkout/…`
- **通知（notification）**
  - `unreadNotifications(): Promise<number>` · L1306 · `/platform/notification/unread`
  - `listNotifications(page = 1, pageSize = 20): Promise<NotificationItem[]>` · L1310 · `/platform/notification/list?page=…`
  - `markNotificationsRead(notificationIds?: string[], all = false): Promise<void>` · L1314 · `/platform/notification/mark-read`
- **模型（model：目录 + 我的模型 + 发布）**
  - `modelList(input: { type?: string, family?: string, category?: string, cursor?: string, limit?: number } = {}): Promise<{ items: ModelListItem[], nextCursor?: string }>` · L1319 · `/platform/model/list…`
  - `modelGet(id: string): Promise<ModelDetail>` · L1328 · `/platform/model/get?id=…`
  - `modelFacets(): Promise<ModelFacets>` · L1331 · `/platform/model/facets`
  - `mineModels(state?: string, cursor?: string, limit = 50): Promise<{ items: MineListItem[], nextCursor?: string }>` · L1334 · `/platform/model/mine/list…`
  - `mineModel(id: string): Promise<MineModel>` · L1340 · `/platform/model/mine/get?id=…`
  - `createModel(): Promise<MineModel>` · L1343 · `/platform/model/create`
  - `saveModel(id: string, draft: ModelDraftInput): Promise<MineModel>` · L1346 · `/platform/model/save`
  - `submitModel(id: string): Promise<MineModel>` · L1349 · `/platform/model/submit`
  - `withdrawModel(id: string): Promise<MineModel>` · L1352 · `/platform/model/withdraw`
  - `setModelHidden(id: string, hidden: boolean): Promise<MineModel>` · L1355 · `/platform/model/setHidden`
  - `removeModel(id: string): Promise<void>` · L1358 · `/platform/model/remove?id=…`
  - `modelCatalog(input: { type?: string, family?: string, category?: string } = {}): Promise<{ items: ModelListItem[] }>` · L1362 · `/platform/model/catalog…` — 模型目录（公开，按类型/族/分类过滤）
  - `prepareModelFile(modelId: string, name: string, bytes: number): Promise<{ id: string, url: string, expiresAt: string }>` · L1373 · `/platform/model/file/prepare` — prepareModelFile 模型文件上传第一步：后端签发限时 PUT 地址
  - `completeModelFile(modelId: string): Promise<{ ok: boolean }>` · L1377 · `/platform/model/file/complete` — completeModelFile 模型文件上传收尾（后端校验对象确实落地后置 ok）
  - `subscribeTaskEvents(onEvent: (event: string, data: Record<string, unknown>) => void): ()` · L1390 — subscribeTaskEvents 订阅平台任务事件流（GET /api/v1/platform/events，产品中立 SSE）
