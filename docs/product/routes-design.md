...
- [ ] 有唯一 path；刷新后能恢复同一视图
- [ ] 页面级 tab/来源/分类全部是 path 子路由，不是组件状态
- [ ] query 只用于筛选/分页/对象选择（§4）
- [ ] 登录/权限/年龄门处理在该页自己的入口，不靠外壳兜底
- [ ] 加载 / 空 / 错误 / 重试 四态独立
- [ ] 跨页返回目标明确（列表→详情→返回列表保留筛选）
- [ ] 分享链接不含临时签名、token、内部 object key
- [ ] 旧 URL 有重定向，不产生 404
- [ ] 页面标题与 `app.vue` 的 `PAGE_NAMES` 映射同步
- [ ] 索引更新：新增页面后跑 `npm run index:frontend`
